...
MLPerceptron::MLPerceptron() :
    eta(INIT_ETA), max_iterations(INIT_MAX_ITER), numHiddenLayers(INIT_NUM_HIDDEN), beta(INIT_BETA), tol(
        INIT_TOL), alpha(INIT_ALPHA) {
...
MLPerceptron::MLPerceptron(size_t numHiddenLayers) :
    eta(INIT_ETA), max_iterations(INIT_MAX_ITER), numHiddenLayers(numHiddenLayers), beta(INIT_BETA), tol(
        INIT_TOL), alpha(INIT_ALPHA) {
  if (numHiddenLayers < 1)
    throw "Invalid number of MLP hidden layers";
  weights = NULL;
...
void MLPerceptron::recall(size_t outputDim, size_t inputDim, DataSet<> &inputs, size_t input_row,
    DataSet<double> &activation) {
  // dim of activation = [numHiddenLayers+1][targetDim]
  // first layer
  recallLayer(outputDim, inputDim, inputs.getRow(input_row), activation.getRow(0), weights[0]);
  // rest of hidden layers and output layer
  for (size_t i = 1; i < numHiddenLayers + 1; i++) {
    recallLayer(outputDim, outputDim, activation.getRow(i - 1), activation.getRow(i), weights[i]);
  }
}

void MLPerceptron::recallLayer(size_t outputDim, size_t inputDim, double *inputs,
    double *activation, DataSet<> *weight) {
...
double MLPerceptron::test(DataSet<> &inputs, DataSet<> &outputs) {
  ...
  accuracy *= 100;
  cout << "Classification Accuracy: " << testsCorrect << "/" << numVectors << " = " << accuracy
      << "%" << endl;
  return accuracy;
}
...
void MLPerceptron::train(DataSet<> &inputs, DataSet<> &outputs, bool randomize_rows) {
...
//            cout << "error\n" << error << endl;
      ...
      for (size_t w = numHiddenLayers; w > 0; w--) {
        ...
        for (size_t j = 0; j < targetDim; j++) {
          for (size_t k = 0; k < targetDim + 1; k++) {
            ...
            double newWeight = currentWeight + eta * error.get(w, j) * activation.get(w, j);
            double momentum = (newWeight - old_weights[w]->get(j, k)) * alpha;
      ...
      for (size_t j = 0; j < targetDim; j++) {
        for (size_t k = 0; k < inputDim + 1; k++) {
          ...
          double newWeight = currentWeight + eta * error.get(0, j) * activation.get(0, j);
          double momentum = (newWeight - old_weights[0]->get(j, k)) * alpha;
