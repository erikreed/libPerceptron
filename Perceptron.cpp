...
inline void Perceptron::recall(size_t outputDim, size_t inputDim, DataSet<> &inputs,
    size_t input_row, double *activation) {
  // compute activations
...
double Perceptron::test(DataSet<> &inputs, DataSet<> &outputs) {
  ...
  if (inputs.rows != outputs.rows) {
    cerr << "number of test cases different between in/out: " << endl;
    throw 1;
  }
  if (weights == NULL) {
    cerr << "Perceptron has not been trained yet." << endl;
    throw 1;
  }
      ...
      for (size_t j = 0; j < targetDim; j++) {
        for (size_t k = 0; k < inputDim; k++) {
          ...
          double newWeight = oldWeight
              + eta * (outputs.get(i, j) - activation[j]) * inputs.get(i, k);
          diff += oldWeight - newWeight > 0 ? oldWeight - newWeight : -(oldWeight - newWeight);
          weights->set(j, k, newWeight);
        }

        // bias term
        double oldWeight = weights->get(j, inputDim);
        double newWeight = oldWeight + eta * (outputs.get(i, j) - activation[j]) * -1;
        diff += oldWeight - newWeight > 0 ? oldWeight - newWeight : -(oldWeight - newWeight);
